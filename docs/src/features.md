...
- [ ] OpenID for Verifiable Credential Issuance (OID4VCI) standards authorization code flow with a mobile wallet
  ...
  - [x] Credential Request with authorization token and proof of possession - WIP: PoP from our e2e wallet is incompatible with the PoP as unime core wants it.
  - [x] Credential Response with signed Verifiable Credential in Open Badges 3.0 format

- [x] Create OpenBadgeCredential Credential Template on the server in admin_cli

- [x] OfferID must be autogenerated in offer service with uuidv4
- [x] Create offer must take an awardID and fetch that from the award service
- [x] Create offer must take a "type", for now only "OpenBadge" and use that to set the credential_configuration_id when creating offer+credential in ssi-agent
- [ ] Create Offer Endpoint must adhere to https://w3c.github.io/vcalm/
