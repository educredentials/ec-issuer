...
- [x] Create OpenBadgeCredential Credential Configuration on the server in admin_cli?
...
- [ ] Create offer must take a "type", for now only "OpenBadge" and use that to set the credential_configuration_id when creating offer+credential in ssi-agent
- [ ] Create credential-configuration must explicitly set `pre_authorized: false`.
